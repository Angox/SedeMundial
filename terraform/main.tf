...
# ==========================================
# 2. IAM - Equipo y Consola (SOLUCIÓN A TU PEDIDO)
# ==========================================

# Grupo de desarrolladores con acceso a Consola
resource "aws_iam_group" "developers" {
  name = "stadiums-dev-team"
}

# Permiso PowerUser (Casi admin, pero sin tocar facturación ni crear usuarios)
...
# Perfil de Login (Permite entrar a la web, requiere cambiar password al primer login)
resource "aws_iam_user_login_profile" "member_1_login" {
  user    = aws_iam_user.member_1.name
  pgp_key = "keybase:terraform" # Truco: usaremos output para ver password o gestión manual
  # NOTA: Terraform requiere PGP para devolver passwords seguros. 
  # SI ESTO TE DA ERROR, BORRA ESTE BLOQUE 'aws_iam_user_login_profile' 
  # y crea las contraseñas manualmente en la consola AWS después de aplicar.
  lifecycle {
...
resource "null_resource" "build_docker" {
  ...
  triggers = { always_run = timestamp() } # Fuerza rebuild siempre que ejecutes
...
resource "aws_iam_role_policy_attachment" "lambda_s3_full" {
  role = aws_iam_role.lambda_role.name; policy_arn = "arn:aws:iam::aws:policy/AmazonS3FullAccess"
}

# Lambda 1: Ingesta (Kaggle -> S3 Raw)
...
resource "aws_lambda_function" "cleaner" {
  ...
  # MAGIA AQUÍ: Usamos la misma imagen, pero diferente comando
  image_config {
...
# Trigger: Cuando llega algo a RAW/, se activa Cleaner
...
# ==========================================
# 5. Redshift Serverless (Liviano y Moderno)
# ==========================================
...
resource "aws_redshiftserverless_workgroup" "stadiums" {
  ...
  base_capacity  = 8 # Mínimo posible (barato)
